...
# ==========================================
# 3. HÀM TÍNH TOÁN LÕI (ĐÃ SỬA LỖI)
# ==========================================
# Giới hạn an toàn: tránh vòng lặp Sinc quá lớn treo app
...
def calculate_signals(f_sig, f_samp, wave_type):
    ...
    # --- FIX SINC BOUNDARY: zero-pad t_disc ra ngoài [0,1] ---
    # Mở rộng cả hai phía để tránh lỗi biên
    pad = min(int(f_samp), 50)  # Thêm tối đa 50 mẫu mỗi phía
    t_disc_pad = np.arange(-pad * T_s, 1 + (pad + 1) * T_s, T_s)
    x_disc_pad = make_wave(t_disc_pad, f_sig, wave_type)

    # Giới hạn số mẫu để tránh treo app (FIX PERFORMANCE)
    if len(t_disc_pad) > MAX_SINC_SAMPLES:
        # Subsample đều để giữ tính đại diện
    ...
    # Vectorized Sinc Interpolation (nhanh hơn vòng lặp for)
    # Shape: (num_points, num_samples)
    t_matrix = (t_cont[:, np.newaxis] - t_disc_pad[np.newaxis, :]) / T_s
    x_recon = x_disc_pad @ np.sinc(t_matrix).T  # dot product vectorized

    # --- FFT (chỉ dùng t_disc trong [0,1]) ---
    N = len(x_disc)
    freqs_full = np.fft.fftfreq(N, T_s)
    fft_full   = np.abs(np.fft.fft(x_disc)) / N

    # Phổ hai phía (two-sided) — để minh họa aliasing folding
    freqs_twosided = np.fft.fftshift(freqs_full)
    fft_twosided   = np.fft.fftshift(fft_full)

    # Phổ một phía (positive)
    pos_mask = freqs_full >= 0

    # --- Sai số & SNR ---
    # So sánh trên cùng lưới t_cont
...
with tab3:
    ...
    ax3a.plot(t_c, x_c,  color=COLOR_ORIG,  linewidth=2, label='Gốc $x(t)$')
    ax3a.plot(t_c, x_r,  color=COLOR_RECON, linewidth=2, label='Khôi phục $x_r(t)$', linestyle='--')
    ax3a.set_xlim(0, 1); ax3a.set_ylim(-1.6, 1.6)
    ax3a.legend(fontsize=9)
    style_ax(ax3a, 'So sánh tín hiệu', COLOR_ORIG)

    ax3b.fill_between(t_c, error, color=COLOR_ERR, alpha=0.6)
    ax3b.axhline(0, color='black', linewidth=0.8)
    ax3b.set_xlim(0, 1)
    style_ax(
        ax3b,
        f'Sai số tái tạo $e(t) = x(t) - x_r(t)$  |  MSE = {mse:.4f}  |  SNR = {snr_db:.1f} dB',
        COLOR_ERR,
    )
    ...
    metrics_df = pd.DataFrame({
        'Chỉ số': ['MSE (Mean Squared Error)', 'SNR (dB)', 'Biên độ lỗi tối đa', 'Biên độ lỗi trung bình'],
        'Giá trị': [f"{mse:.6f}", f"{snr_db:.2f} dB", f"{np.max(np.abs(error)):.4f}", f"{np.mean(np.abs(error)):.4f}"],
        'Nhận xét': [
            'Càng nhỏ càng tốt',
            '> 20 dB: Tốt | 5-20 dB: Trung bình | < 5 dB: Kém',
            'Sai số cực đại tại một điểm',
            'Sai số trung bình toàn tín hiệu',
        ]
    })
...
with tab5:
    ...
    ax5.axvline( nyquist_freq, color=COLOR_NYQ, linestyle='--', linewidth=2, label=f'+Nyquist = +{nyquist_freq:.1f} Hz')
    ax5.axvline(-nyquist_freq, color=COLOR_NYQ, linestyle='--', linewidth=2, label=f'−Nyquist = −{nyquist_freq:.1f} Hz')
